...
int serviceGetInfoFromServiceHandle(SC_HANDLE schService, LPCTSTR lpServiceName);
...
int serviceGetInfoFromManagerHandle(SC_HANDLE schManager, LPCTSTR lpServiceName)
{
	...
	if( NULL == (schService = OpenService( schManager, lpServiceName, SERVICE_QUERY_STATUS | SERVICE_QUERY_CONFIG )) ) 
	{
		switch( GetLastError() ) 
		{
	...
	{
		iReturnCode = serviceGetInfoFromServiceHandle(schService,lpServiceName);
	}
